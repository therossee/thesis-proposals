...
export default function ThesisSteps({ activeStep }) {
  ...
  const renderStep = (step, index) => {
    ...
    return (
      <div key={step.key} id={index} className="progress-step">
        <div className="progress-step-marker">
          <div className={`progress-step-circle ${isActive ? 'pending' : 'inactive'}`}></div>
        </div>
        <div className="progress-step-content">
          <h6 className={`progress-step-title ${isActive ? 'active' : 'inactive'}`}>{step.label}</h6>
