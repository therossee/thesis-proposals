...
import { FaRegCircleQuestion, FaArrowsRotate } from 'react-icons/fa6';
  ...
  return (
    <Card className="mt-3 roundCard">
      <Card.Body className="d-flex flex-column align-items-center my-4">
        ...
        <Button className={`btn-${appliedTheme}`} size="sm" onClick={() => window.location.reload()}>
          <FaArrowsRotate size={16} />
          {t('carriera.tesi.reload_page')}{' '}
        </Button>
      </Card.Body>
    </Card>
  );
